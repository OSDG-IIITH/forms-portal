<script lang="ts">
  ...
  let form = {
    title: data.templateTitle || "Untitled Form",
    description: data.templateDescription || "Add a description",
    visibility: "private",
    structure: data.templateKdl || "",
  };
  async function handleCreate(event: CustomEvent) {
    ...
    for (let attempt = 0; attempt < 100; attempt++) {
      const payload = {
        ...
        max_responses:
          formData.max_responses !== null &&
          formData.max_responses !== undefined
      ...
      if (res.ok) {
        ...
        let handle;
        try {
          const userRes = await fetch("/api/auth/info", {
            credentials: "include",
          });
          if (userRes.ok) {
      ...
      toast.error("Error creating form", {
        description: text || "Failed to create form.",
      });
      return;
    }
    toast.error("Error creating form", {
      description:
        "Could not generate a unique title/slug after many attempts.",
    });
  }
</script>
      ...
      <div class="text-center py-12">
        ...
        <div class="mt-8 flex flex-col items-center gap-4">
          <CopyButton text={fullSharableLink} size="sm" variant="outline">
            <span class="font-mono text-sm font-light truncate"
              >{fullSharableLink}</span
            >
            {#snippet icon()}
          ...
          <div class="flex items-center gap-4">
            ...
            <Button href={sharableLink} variant="secondary" target="_blank"
              >View Form</Button
            >
          </div>
        </div>
