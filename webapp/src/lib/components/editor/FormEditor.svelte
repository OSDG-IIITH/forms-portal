<script lang="ts">
  ...
  const questionTypeButtons = [
    ...
    {
      type: "section-header" as QuestionType,
      icon: IconHeading,
    ...
    try {
      ...
      const payload = {
        ...
        closes: formatDateTime(
          store.formData.closes,
          store.formData.closesTime,
        ),
        anonymous: Boolean(store.formData.anonymous),
        max_responses:
          store.formData.max_responses !== null &&
          store.formData.max_responses !== undefined
