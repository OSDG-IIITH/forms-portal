<script lang="ts">
  ...
  const activeKeys = $derived.by(() => {
    ...
    if (question.placeholder !== undefined) keys.push('placeholder');
    if ('min-chars' in (question.validations ?? {}) || 'max-chars' in (question.validations ?? {})) {
      keys.push('charlimit');
    }

    const rt = regexType;
    if (rt === 'number') keys.push('number');
    if (rt === 'email') keys.push('email');
    if (rt === 'regex') keys.push('regex');
  ...
  const availableToAdd = $derived(chips.filter((v) => !activeKeys.includes(v.key)));
